<%@ Page Title="Oklahoma High School Wrestling Dual State" Language="vb" AutoEventWireup="false" MasterPageFile="MasterPageWrestlingNOAJAX.Master" CodeBehind="WRDStateTourneyScores.aspx.vb" Inherits="TheSportsEngine2017.WRDStateTourneyScores" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
                    ...
                    <telerik:LayoutColumn Span="6" SpanXl="6" SpanLg="6" SpanMd="12" SpanSm="12" SpanXs="12">
                        <asp:Panel ID="RadAjaxPanel100" runat="server" CssClass="grid_wrapper">
                            <asp:Table runat="server" ID="tblBKBStateTourneyScores">
                                ...
                                <asp:TableRow>
                                    <asp:TableCell>
                                        <br /><asp:Label runat="server" ID="tblFooter" Font-Size="X-Small" Text="Data provided by Mark Kirk"></asp:Label>
                                    </asp:TableCell>
                                </asp:TableRow>
                            </asp:Table>  
                        </asp:Panel>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT *, strRound + ' : ' + strDisplay AS strDisplayRound FROM IWATGview_WrestlingDual_StateTournamentGames_ALL WHERE strState = @state AND strClass = @selectedClass AND intYear = @selectedYear ORDER BY intSortRound, intGameNo">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT intYear, CAST(intYear AS varchar) AS strYear FROM IWATGtmpview_WrestlingDualGameWinnersAndLosers_ALL WHERE strState = @state ORDER BY intYear DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
        </SelectParameters>    
    </asp:SqlDataSource>    
    ...
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT strClass FROM IWATGtmpview_WrestlingDualGameWinnersAndLosers_ALL WHERE intYear = @selYear ORDER BY strClass DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
