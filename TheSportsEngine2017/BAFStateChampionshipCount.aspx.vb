...
Partial Class BAFStateChampionshipCount
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("globalUser") = "" Then
            'SqlDataSource1.SelectCommand = "SELECT DISTINCT strWinner, CAST(intStateChampionshipCount AS varchar) + ' - ' + strWinner + ' (' + CAST(intLastChampionshipYear AS varchar) + ')' AS strDisplay, intStateChampionshipCount FROM IWATGview_BaseballFall_StateChampions WHERE strState = @state AND intStateChampionshipCount >=6 ORDER BY intStateChampionshipCount DESC, strWinner"
            SqlDataSource1.SelectCommand = "SELECT DISTINCT strWinner, CAST(intStateChampionshipCount AS varchar) + ' - ' + strWinner + ' (' + CAST(intLastChampionshipYear AS varchar) + ')' AS strDisplay, intStateChampionshipCount FROM IWATGview_BaseballFall_StateChampions WHERE strState = @state ORDER BY intStateChampionshipCount DESC, strWinner"
        Else
            SqlDataSource1.SelectCommand = "SELECT DISTINCT strWinner, CAST(intStateChampionshipCount AS varchar) + ' - ' + strWinner + ' (' + CAST(intLastChampionshipYear AS varchar) + ')' AS strDisplay, intStateChampionshipCount FROM IWATGview_BaseballFall_StateChampions WHERE strState = @state ORDER BY intStateChampionshipCount DESC, strWinner"
        End If
