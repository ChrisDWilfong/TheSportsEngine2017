<%@ Page Title="Oklahoma High School Baseball Playoffs" Language="vb" AutoEventWireup="false" MasterPageFile="MasterPageBaseballNOAJAX.Master" CodeBehind="BAStateTourneyScores.aspx.vb" Inherits="TheSportsEngine2017.BAStateTourneyScores" %>
...
<asp:Content ID="Content0" ContentPlaceHolderID="head" Runat="Server">
    ...
    <meta name="twitter:title" content="Oklahoma High School Baseball Tournament Scores" />
    <meta name="twitter:description" content="ALL-TIME High School Baseball State Tournment Scores (since 1947)" />
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
            ...
            <telerik:LayoutRow>
                <Columns>
                    ...
                    <telerik:LayoutColumn Span="6" SpanXl="6" SpanLg="6" SpanMd="12" SpanSm="12" SpanXs="12">
                        <asp:Panel ID="RadAjaxPanel1000" runat="server" CssClass="grid_wrapper">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT *, strRound + ' : ' + strDisplay AS strDisplayRound FROM IWATGview_Baseball_StateTournamentGames_ALL WHERE strState = @state AND strClass = @selectedClass AND intYear = @selectedYear ORDER BY intSortRound, intGameNo">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT intYear, CAST(intYear AS varchar) AS strYear FROM tblYears WHERE strSport = 'Baseball' AND strState = @state ORDER BY intYear DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
        </SelectParameters>    
    </asp:SqlDataSource>    
    ...
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT strClass FROM tblBaseballTournament WHERE strState = @state AND intYear = @selYear ORDER BY strClass DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
