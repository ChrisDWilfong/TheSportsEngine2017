...
namespace aspnetforum.Utils
{
	...
	public class DbAwareSettings
	{
		...
		private static readonly Dictionary<string, string> _defaults = new Dictionary<string, string>()
		{
			...
			{ "EnableRating", "true" },
			{ "ForumTitle", "State Title Talk<br>SPONSORED BY : FCA" },
			{ "TitleLink", "http://www.iwasatthegame.com/titletalk/Default.aspx" },
			{ "MailServer", "SMTPauth.earthlink.net" },
			{ "MailServerPort", "25" },
			{ "MailServerLogin", "iwasatthegame@earthlink.net" },
			{ "MailServerPassword", "Iluvln143!" },
			{ "MailFromAddress", "chris@iwasatthegame.com" },
			{ "MailUseSSL", "false" },
			{ "SendErrorReportsTo", "" },
			{ "EmailDebug", "false" },
...
EXAMPLE 3: you have no parent website at all, but you have configured the forum to use windows-authentication (above). AspNetForum will detect current authenticated windows-users and add log them in automatically (and add them to the forum users database)." },
		...
		You have to register an 'app' at http://developers.facebook.com/setup/ to obtain these keys" },
			...
			{ "DisableExternalLinks", "Prohibit posting external links (receommended to prevent spam)" }
		};
	...
	public static class Settings
	{
		...
		public static bool DisableExternalLinks
		{
			get
			{
				return (DbAwareSettings.Current["DisableExternalLinks"].ToLower() == "true");
			}
		}
	}
}
