...
namespace aspnetforum.Utils
{
	public static class Topic
	{
		...
		public static void MergeTopics(int topicIdToMergeInto, int topicIdBeingMerged, out string newTopicSubject)
		{
			if (topicIdToMergeInto == topicIdBeingMerged)
			{
				newTopicSubject = "";
				return;
			}
			...
			using (var cn = DB.CreateOpenConnection())
			{
				cn.ExecuteNonQuery("UPDATE ForumMessages SET TopicID=? WHERE TopicID=?", topicIdToMergeInto, topicIdBeingMerged);

				cn.ExecuteNonQuery(@"UPDATE ForumSubscriptions SET TopicID=?
					WHERE TopicID=? AND UserID NOT IN (SELECT UserID FROM ForumSubscriptions WHERE TopicID=?)", topicIdToMergeInto, topicIdBeingMerged, topicIdToMergeInto);

				cn.ExecuteNonQuery("DELETE FROM ForumSubscriptions WHERE TopicID=?", topicIdBeingMerged);

				cn.ExecuteNonQuery("UPDATE ForumPolls SET TopicID=? WHERE TopicID=?", topicIdToMergeInto, topicIdBeingMerged);

				cn.ExecuteNonQuery("DELETE FROM ForumTopics WHERE TopicID=?", topicIdBeingMerged);
