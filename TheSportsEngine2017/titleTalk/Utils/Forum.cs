...
namespace aspnetforum.Utils
{
	public static class Forum
	{
		...
		public static DataSet GetForumsForFrontpage(int userId, int groupID)
		{
		...
		public static IEnumerable<int> GetReadableForumsForUser(int userId)
		{
			...
			using (DbConnection cn = DB.CreateOpenConnection())
			{
				if (userId == 0) //return "anonymuous" forums for non-authenticated users
					return cn.ExecuteOrm<int>(@"SELECT ForumID FROM Forums WHERE ForumID NOT IN (SELECT ForumID FROM ForumGroupPermissions WHERE AllowReading=?) AND MembersOnly=?", true, false);
