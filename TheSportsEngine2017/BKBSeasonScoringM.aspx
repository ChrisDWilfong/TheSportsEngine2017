<%@ Page Title="Oklahoma High School Basketball Season Scoring" Language="vb" AutoEventWireup="false" MasterPageFile="MasterPageBasketball.Master" CodeBehind="BKBSeasonScoringM.aspx.vb" Inherits="TheSportsEngine2017.BKBSeasonScoringM" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT TOP 25 CAST(intStat AS varchar(10)) + ' - ' + strPlayer + ' (' + strSchool + '-' + strClass + ') ' + CASE WHEN strNotes IS NULL THEN '' ELSE '<br>' + strNotes END AS strDisplay, * FROM tblBasketballStats WHERE strState = @state AND intYear = @selectedYear AND strGender = 'Boys' AND strType = 'POINTS' ORDER BY intStat DESC">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT intYear, CAST(intYear AS varchar) AS strYear FROM tblBasketballStats WHERE strGender = 'Boys' AND strState = @state ORDER BY intYear DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
        </SelectParameters>    
    </asp:SqlDataSource>    
