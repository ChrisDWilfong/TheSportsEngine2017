<%@ Page Title="Oklahoma High School Girls Basketball Playoffs" Language="vb" AutoEventWireup="false" MasterPageFile="MasterPageBasketballNOAJAX.Master" CodeBehind="BKGStateTourneyScores.aspx.vb" Inherits="TheSportsEngine2017.BKGStateTourneyScores" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
            ...
            <telerik:LayoutRow>
                <Columns>
                    ...
                    <telerik:LayoutColumn Span="6" SpanXl="6" SpanLg="6" SpanMd="12" SpanSm="12" SpanXs="12">
                        ...
                        <asp:Table runat="server" ID="tblBKGStateTourneyScores">
                            <asp:TableRow>
                                <asp:TableCell>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT *, strRound + ' : ' + strDisplay AS strDisplayRound FROM IWATGview_Basketball_StateTournamentGames_ALL WHERE strSportGenderKey = 'BasketballGirls' AND strState = @state AND strClass = @selectedClass AND intYear = @selectedYear ORDER BY intSortRound, intGameNo">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT intYear, CAST(intYear AS varchar) AS strYear FROM IWATGtmpview_GirlsGameWinnersAndLosers_ALL WHERE strState = @state ORDER BY intYear DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
        </SelectParameters>    
    </asp:SqlDataSource>    
    ...
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT strClass FROM IWATGtmpview_GirlsGameWinnersAndLosers_ALL WHERE strState = @state AND intYear = @selYear ORDER BY strClass DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
