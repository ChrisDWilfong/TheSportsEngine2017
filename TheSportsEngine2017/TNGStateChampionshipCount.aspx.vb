...
Partial Class TNGStateChampionshipCount
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Else
        '    SqlDataSource1.SelectCommand = "SELECT DISTINCT strWinner, CAST(intStateChampionshipCount AS varchar) + ' - ' + strWinner + ' (' + CAST(intLastChampionshipYear AS varchar) + ')' AS strDisplay, intStateChampionshipCount FROM IWATGview_Tennis_StateChampions WHERE strState = @state AND strSportGenderKey = 'TennisGirls' ORDER BY intStateChampionshipCount DESC, strWinner"
        'End If
        ...
        lblOneColumnDetail.Text = "GIRLS TENNIS" & clsStateChampions.GetSinceFirstYear(Session("globalState"), Session("globalSport"))
        lblOneColumnHeader0.Text = lblOneColumnHeader.Text
        lblOneColumnDetail0.Text = lblOneColumnDetail.Text

        Try
            clsLog.LogEvent("TENNISGIRLS", Me.Page.AppRelativeVirtualPath.ToString, "", "", Request.UserHostAddress, Session("globalState"), Session("globalUser"), Request.UserAgent)
        Catch
        End Try
