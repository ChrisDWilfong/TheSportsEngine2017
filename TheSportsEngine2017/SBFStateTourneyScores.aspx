<%@ Page Title="Oklahoma High School Fast Pitch Softball Playoffs" Language="vb" AutoEventWireup="false" MasterPageFile="MasterPageSoftball.Master" CodeBehind="SBFStateTourneyScores.aspx.vb" Inherits="TheSportsEngine2017.SBFStateTourneyScores" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder2" runat="Server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT *, strRound + ' : ' + strDisplay AS strDisplayRound FROM IWATGview_FPSoftball_StateTournamentGames_ALL WHERE strState = @state AND strClass = @selectedClass AND intYear = @selectedYear ORDER BY intSortRound, intGameNo">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:IWATGConnectionString %>" 
        SelectCommand="SELECT DISTINCT intYear, CAST(intYear AS varchar) AS strYear FROM IWATGview_FPSoftball_StateTournamentGames_ALL WHERE strState = @state ORDER BY intYear DESC">
        <SelectParameters>
            <asp:SessionParameter Name="state" SessionField="globalState" DefaultValue="OK" Type="String" />
        </SelectParameters>    
    </asp:SqlDataSource>    
